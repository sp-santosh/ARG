...
import { fetchSubjects } from "../utils/auth.api";
...
const SubjectsListing = () => {
  ...
  const updateModalAndSelectId = (id: string) => {
    setShowModal(true);
    setSelectedID(id);
  };

  const onDeleteCall = async (e: any) => {
    e.preventDefault();
    try {
      const response = await authHttp({
        url: `/api/subjects/${selectedID}`,
        method: "DELETE",
      });
      ...
      <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
        <div className="max-w-full overflow-x-auto">
          <table className="w-full table-auto">
            ...
            <tbody>
              {isLoading ? (
                <p className="min-w-[150px] py-4 px-4 font-medium text-black dark:text-white">
                  "LOADING"
                </p>
              ) : (
                    ...
                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                      <div className="flex items-center space-x-3.5">
                        <button
                          ...
                          onClick={() => push(`/subjects/${subject.id}`)}
                        >
                          <svg
                        ...
                        <button
                          className="hover:text-primary"
                          onClick={() => updateModalAndSelectId(subject.id)}
                        >
                          <svg
                            className="fill-current"
                            width="18"
