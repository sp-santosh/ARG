...
export class RoutineController {
      ...
      for (let dayCount = 0; dayCount < 6; dayCount++) {
        ...
        for (let sectionCount = 0; sectionCount < 3; sectionCount++) {
          for (let slotCount = 0; slotCount < 4; slotCount++) {
            ...
            const facultyName_slot =
              await facultyRepositoryInstance.findFacultyById(facultyId);

            let token = 1;
            while (token === 1) {
              // console.log("inside while");
              // console.log("locationCount:" + locationCount);
              const slotObj = await slotRepositoryInstance.findSlotById(
                slotCount
              );
              ...
              const collegeData =
                await newCollegeRepositoryInstance.findCollegeById(reference);
              console.log("College Data: =======" + collegeData.id);
              const facultyName_db = collegeData.faculty;

              const determineFacultyCode =
                await facultyRepositoryInstance.findByClassName(facultyName_db);
              ...
              if (
                ...
                if (facultyName_slot.className === facultyName_db) {
                  let teacherStartTime;
                  let teacherEndTime;
                  if (locationCount >= 12 && locationCount <= 23) {
                    teacherStartTime = collegeData.tueStartTime;
                    teacherEndTime = collegeData.tueEndTime;
                  } else if (locationCount >= 12 && locationCount <= 23) {
                  ...
                  console.log("teacherStartTime: " + teacherStartTime);
                  console.log("teacherEndTime: " + teacherEndTime);
                  const allocatedRepetition = collegeData.classesPerWeek;
                  if (
                    teacherStartTime <= startTime &&
                    teacherEndTime >= endTime
                  ) {
                    // console.log("workAllocated--------------");
                    const cSubjectCode = collegeData.subjectCode;
                    const cTeacherCode = collegeData.teacherCode;
                    // collegeData.save(collegeData);
                    // locationCount = 0;
                    chromosomeList.push(
                      `${facultyCode}.${cTeacherCode}.${cSubjectCode}.${slotCode}`
                    );
                    console.log("List: " + chromosomeList);
                    break;
                  }
                }
              } else {
  ...
  async splitDna(id) {
    try {
      ...
      let chromos = chromosome.chromo.substring(
        1,
        chromosome.chromo.length - 1
      );
  ...
  async hardConstraintH1() {
    ...
    let initial = 0;
    for (l = 0; l < 6; l++) {
      //routine of 6 days a week
      for (i = initial; i < initial + 4; i++) {
        ul = ll + 8;
        for (j = ll; j <= ul; j = j + 4) {
          for (k = j + 4; k <= ul; k = k + 4) {
            //+4 because same slot repeats after duration of 4
  ...
  async hardConstraintH3(fitnessValue) {
    ...
    for (let count = 0; count < size; count++) {
      ...
      if (
        workAllocated === comparisonCount - 1 ||
        workAllocated === comparisonCount ||
        workAllocated === comparisonCount + 1
      ) {
        fitnessValue = fitnessValue + 10;
      } else {
  ...
  async crossOver() {
    ...
    while (optimumFitness < threshold) {
      ...
      for (let i = 0; i < 2; i++) {
        let chromos = chromo[i]
          .substring(1, chromo[i].length - 1)
          .replace(/\s/g, "");
      ...
      for (let i = 0; i < 72; i++) {
        ...
        // console.log("randnumFitness1 :" + randnumFitness1);
        let randnumFitness2 = (multiplier * seed2 + increment) % modulus;
        // console.log("randnumFitness2 :" + randnumFitness2);
        ...
        if (fitnessDna1 && fitnessDna2) {
          // console.log("value found " + fitnessDna1 + " " + fitnessDna2);
          ...
          let collegeRepo = new CollegeRepository();
          // console.log("DNAF1 :" + dnaF1 + "DNAF2 :" + dnaF2);
          if (dnaF1 === dnaF2) {
            // console.log("inside if crossover");
            if (indicator1 === 1 || indicator2 === 1) {
              if (randnum1 < 3) {
                let collegeReference1 = await collegeRepo.findByTeacherCode(
                  dnaT1
                );
                let tStart1, tEnd1;
                ...
                let collegeReference2 = await collegeRepo.findByTeacherCode(
                  dnaT2
                );
                let tStart2, tEnd2;
      ...
      await this.createChildChromosome(
        totalFitness1,
        totalFitness2,
        hfitnessValue1,
        sfitnessValue1,
    ...
    for (let mutatechromosome = 0; mutatechromosome < 2; mutatechromosome++) {
      for (let i = 0; i < 72; i++) {
        ...
        if (fitness11 && fitness22) {
          ...
          if (mutatechromosome === 1) {
            //mutation operation for chromosome in fitness2
          ...
          if (randnum2 < 1) {
            // console.log("mutation started!!");    //mutation operation if it is within the mutation probability i.e. 20%
            ...
            while (token === 1) {
              if (facultyCollege !== facultyName) {
                //apply mutation only if faculty matches
  ...
  async storeCode(optimumFitness) {
    ...
    for (let i = 0; i < gene.length; i++) {
      ...
      finalDna.faculty = dna[0].replace(/"/g, "");
      finalDna.teacher = dna[1];
      finalDna.subject = dna[2];
      finalDna.slot = dna[3].replace(/"/g, "");
      finalDna.token = i;
  ...
  async storeActualData() {
    ...
    for (let i = 0; i < 72; i++) {
      ...
      if (finalDna) {
        ...
        let teacher1 =
          await collegeRepo.findByTeacherCodeAndSubjectCodeAndFaculty(
            teacher,
            subject,
            faculty11
          );
